...
def test_all_states_direct():
    ...
    # Check subdirectory only
    tenok = zerostatus.copy()
    tenok["OK"] = 10
    util.assert_status(r, path="subdir-a", status=tenok)

    # Check again from within the subdir
    r.cdrel("subdir-a")
    util.assert_status(r, status=tenok)
    r.cdrel("..")
    ...
    oneok = zerostatus.copy()
    oneok["OK"] = 1
    # Check one of the files explicitly
    util.assert_status(r, status=oneok, path="root-70.annex")
